...
namespace AI {
    ...
    IOData::loadFromFile(std::string filename) {
    ...
    bool IOData::writeToStream(std::ofstream& filestream) {
	...
	IOData::findNodeIndex(std::string label) {
        for (int i = 0; i < (int)_subNodes.size(); i++) {
            if (_subNodes[i].getName() == label) {
                return i;
            }
        }
        return -1;
