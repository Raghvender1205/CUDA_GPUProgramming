...
def BlackHoleCL(zImage, fImage, InputCL):
    ...
    if not HasXPU:
        print("No XPU #%i found in all of %i devices, sorry..." %
              (Device, Id - 1))
        sys.exit()
    ...
    if Method == "TrajectoPixel" or Method == "TrajectoCircle":
        ...
        IdLastCL = cl.Buffer(ctx, mf.WRITE_ONLY |
                             mf.COPY_HOST_PTR, hostbuf=IdLast)
...
def BlackHoleCUDA(zImage, fImage, InputCL):
    ...
    TrajectoriesCU = cuda.mem_alloc(
        Trajectories.size * Trajectories.dtype.itemsize)
    ...
    if Method == "TrajectoPixel" or Method == "TrajectoCircle":
        if not NoImage:
            ...
            numpy.savetxt(
                "TrouNoirTrajectories.csv",
                numpy.transpose(numpy.concatenate(
                    (Angles, Trajectories), axis=0)),
                delimiter=" ",
                fmt="%.2e",
            )
